...
def render_auth_screen():
    ...
    render_public_tools_section(NAVY, GOLD)


def render_public_tools_section(NAVY: str, GOLD: str):
    """General-purpose tools that don't need a saved account — the current
    sky, today's remedy, daily panchang/hora, and a numerology profile all
    take their own direct input (or none at all) rather than relying on a
    signed-in user's own birth chart. The actual personal Kundali Analysis
    (diamond/wheel charts, dashas, chart library) stays behind sign-in,
    since that's tied to a saved profile."""
    st.markdown("---")
    st.markdown(
        f'<p style="font-family:Georgia,serif;font-size:24px;color:{NAVY};text-align:center;margin:20px 0 4px;">'
        'Try These Without Signing In</p>'
        f'<p style="text-align:center;color:#7a8296;margin-bottom:20px;">'
        'Sign in only when you want your personal Kundali Analysis — everything below is free to explore first.</p>',
        unsafe_allow_html=True,
    )

    pub_tab1, pub_tab2, pub_tab3, pub_tab4 = st.tabs(
        ["\U0001f504 Nakshatra Live & Transits", "\U0001f4ff Day-wise Remedies",
         "\U0001f549\ufe0f Panchang & Muhurta", "\U0001f522 Numerology"]
    )

    with pub_tab1:
        pub_lat, pub_lon, pub_tz, _ = _public_location_picker("pub_transit")
        render_nakshatra_live_clock(pub_lat, pub_lon, pub_tz)
        st.markdown(f'<p style="color:{GOLD};font-weight:700;margin-top:18px;">Current Transits</p>', unsafe_allow_html=True)
        now_local = now_in_city(pub_tz)
        transit_chart = compute_chart(now_local.year, now_local.month, now_local.day,
                                       now_local.hour, now_local.minute, pub_lat, pub_lon, pub_tz)
        t_rows = []
        jd_now = transit_chart["jd"]
        for b in transit_chart["bodies"]:
            if b["key"] not in CORE_KEYS or b["key"] == "As":
                continue
            retro = " \u2118" if (b["retro"] and b["key"] not in ("Ra", "Ke")) else ""
            color = PLANET_TRANSIT_COLORS.get(b["key"], "#333")
            entry_jd, exit_jd = compute_nakshatra_transit_window(b["key"], jd_now, b["nakIdx"])
            entry_str = jd_to_local_date_str(entry_jd, pub_tz) if entry_jd else "\u2014"
            exit_str = jd_to_local_date_str(exit_jd, pub_tz) if exit_jd else "\u2014"
            t_rows.append(
                f'<tr style="background:{color}22;"><td style="font-weight:700;color:{color};padding:6px 10px;">{b["key"]}</td>'
                f'<td style="padding:6px 10px;">{fmt_deg(b["inSign"])}{retro}</td>'
                f'<td style="padding:6px 10px;">{SIGNS[b["sign"]]}</td>'
                f'<td style="padding:6px 10px;">{NAKSHATRAS[b["nakIdx"]]} '
                f'<span style="color:#888;font-size:13px;">({entry_str} \u2013 {exit_str})</span></td></tr>'
            )
        st.markdown(
            '<table style="width:100%;border-collapse:collapse;">'
            '<tr><th style="text-align:left;padding:6px 10px;">Graha</th><th style="text-align:left;padding:6px 10px;">Degree</th>'
            '<th style="text-align:left;padding:6px 10px;">Sign</th><th style="text-align:left;padding:6px 10px;">Nakṣatra</th></tr>'
            + "".join(t_rows) + "</table>",
            unsafe_allow_html=True,
        )
    ...
    with pub_tab2:
        ...
        st.markdown(
            f'<div style="background:#FFFDF3;border:1px solid #F0DE94;border-radius:12px;padding:18px 22px;">'
            f'<p style="font-size:18px;margin:0 0 6px;"><b>{day_name}</b> \u00b7 ruled by {BODY_FULLNAME_ASCII.get(day_key, day_key)}</p>'
            f'<p style="color:#888;margin:6px 0;">Gemstone: {gem} \u00b7 Colour: {colour}</p>'
            f'<p style="margin:10px 0 0;">{remedy}</p></div>',
            unsafe_allow_html=True,
        )

    with pub_tab3:
        pub_lat2, pub_lon2, pub_tz2, pub_city2 = _public_location_picker("pub_panchang")
        pub_date = st.date_input("Date", value=date.today(), key="pub_panchang_date")
        pmw = compute_muhurta_windows(pub_date.year, pub_date.month, pub_date.day, pub_lat2, pub_lon2, pub_tz2)
        pchart = compute_chart(pub_date.year, pub_date.month, pub_date.day, 12, 0, pub_lat2, pub_lon2, pub_tz2)
        pan = pchart["panchanga"]
        st.markdown(
            f'<div class="krow2" style="display:flex;justify-content:space-between;padding:6px 0;border-bottom:1px solid #eee;">'
            f'<span>Tithi</span><span>{pan["paksha"]} {pan["tithiName"]}</span></div>'
            f'<div style="display:flex;justify-content:space-between;padding:6px 0;border-bottom:1px solid #eee;">'
            f'<span>Nakṣatra</span><span>{NAKSHATRAS[pan["nakIdx"]]}</span></div>'
            f'<div style="display:flex;justify-content:space-between;padding:6px 0;border-bottom:1px solid #eee;">'
            f'<span>Yoga</span><span>{YOGAS[pan["yogaIdx"]]}</span></div>'
            f'<div style="display:flex;justify-content:space-between;padding:6px 0;border-bottom:1px solid #eee;">'
            f'<span>Karaṇa</span><span>{pan["karana"]}</span></div>'
            f'<div style="display:flex;justify-content:space-between;padding:6px 0;border-bottom:1px solid #eee;">'
            f'<span>Abhijit Muhurta</span><span>{_fmt_hm(pmw["abhijit"][0])} \u2013 {_fmt_hm(pmw["abhijit"][1])}</span></div>'
            f'<div style="display:flex;justify-content:space-between;padding:6px 0;">'
            f'<span>R\u0101hu K\u0101lam</span><span>{_fmt_hm(pmw["rahu_kalam"][0])} \u2013 {_fmt_hm(pmw["rahu_kalam"][1])}</span></div>',
            unsafe_allow_html=True,
        )
        st.caption(f"For {pub_city2}.")

    with pub_tab4:
        st.markdown("**Enter your details for a numerology snapshot** (Pythagorean & Chaldean):")
        pn1, pn2 = st.columns(2)
        with pn1:
            pub_num_name = st.text_input("Full Name", key="pub_num_name")
        with pn2:
            pub_num_dob = st.date_input("Date of Birth", value=date(1995, 1, 1),
                                         min_value=date(1900, 1, 1), max_value=date(2100, 12, 31), key="pub_num_dob")
        if st.button("Generate", key="pub_num_generate"):
            if not pub_num_name.strip():
                st.error("Enter your name.")
            elif not _clean_name(pub_num_name):
                st.error("Name needs Latin letters (A-Z) for these systems to work.")
            else:
                lp = pythagorean_life_path(pub_num_dob)
                expr = pythagorean_expression(pub_num_name)
                cl = chaldean_life_number(pub_num_dob)
                cn = chaldean_name_number(pub_num_name)
                nc1, nc2 = st.columns(2)
                with nc1:
                    st.markdown(
                        f'<div style="background:#FFFDF3;border:2px solid {GOLD};border-radius:12px;padding:16px;">'
                        f'<b>Pythagorean</b><br>Life Path: <b>{lp}</b> \u2014 {NUMEROLOGY_MEANINGS_PYTHAGOREAN.get(lp,"")}<br>'
                        f'Expression: <b>{expr}</b> \u2014 {NUMEROLOGY_MEANINGS_PYTHAGOREAN.get(expr,"")}</div>',
                        unsafe_allow_html=True,
                    )
                with nc2:
                    st.markdown(
                        f'<div style="background:#FFFDF3;border:2px solid #C0392B;border-radius:12px;padding:16px;">'
                        f'<b>Chaldean</b><br>Life Number: <b>{cl}</b> \u2014 {NUMEROLOGY_MEANINGS_CHALDEAN.get(cl,"")}<br>'
                        f'Name Number: <b>{cn["reduced"]}</b> (compound {cn["compound"]}) \u2014 {NUMEROLOGY_MEANINGS_CHALDEAN.get(cn["reduced"],"")}</div>',
                        unsafe_allow_html=True,
                    )
                st.caption("Sign in and visit Numerology for the full profile (Soul Urge, Personality, Lo Shu Grid, Compatibility, and more).")

    st.markdown(
        '<p style="text-align:center;color:#999;font-size:13px;margin-top:10px;">'
        'Want your personal Kundali Analysis, saved charts, and Navtara Chakra? '
        '<b>Sign in above</b> \u2014 it only takes a moment.</p>',
        unsafe_allow_html=True,
    )


def _public_location_picker(key_prefix: str):
    """A simple city-search location picker for anonymous visitors (no
    saved profile to default from), reused across the public tool tabs."""
    query = st.text_input("Location", value="Bathinda", key=f"{key_prefix}_query")
    matches = [c for c in CITIES if query.lower() in (c[0] + " " + c[1]).lower()] or CITIES[:8]
    labels = [f"{c[0]} \u00b7 {c[1]}" for c in matches[:8]]
    chosen = st.selectbox("Match", labels, key=f"{key_prefix}_sel", label_visibility="collapsed")
    city = matches[labels.index(chosen)]
    return city[2], city[3], city[4], city[0]
